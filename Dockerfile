...
RUN wget --no-cookies \
         ...
         -O /tmp/jdk-8u60-linux-x64.rpm && \
         rpm -ihv /tmp/jdk-8u60-linux-x64.rpm && \
         rm /tmp/jdk-8u60-linux-x64.rpm
...
RUN mkdir -p /opt/hybris/5.6.0 && \
    ln -s /opt/hybris/5.6.0 /opt/hybris/current && \
    unzip /tmp/hybris*.zip -d /opt/hybris/current && \
    rm /tmp/hybris*.zip
...
# Set mountable volumes for license and config
VOLUME ["/opt/hybris/current/hybris/config/license", "/opt/hybris/current/hybris/config"]
